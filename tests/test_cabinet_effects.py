...
class CabinetEffectsTests(unittest.TestCase):
    ...
    def test_regional_portfolios_boost_network(self) -> None:
        ...
        self.assertGreater(cabinet.gray_fund, 0)
        self.assertGreaterEqual(cabinet.corruption_load, 0)

    def test_heavy_corruption_builds_exposure_hooks(self) -> None:
        municipalities, districts = build_default_territory()
        nation = NationState(turn=31, stage="mid_mandate", districts=districts, municipalities=municipalities)
        party = self._party("RiskGov")
        cabinet = CabinetState(
            exists=True,
            pm_party=party.name,
            assignments=[MinisterAssignment(portfolio_id="transport", party_id=party.name, holder_quality=20)],
            budget_load=85,
        )
        ...
        self.assertGreater(cabinet.corruption_load, 0)
        self.assertTrue(len(cabinet.scandal_hooks) >= 1)
